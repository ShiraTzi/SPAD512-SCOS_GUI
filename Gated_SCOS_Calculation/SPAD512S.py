...
class SPAD512S:
    ...
    def get_intensity(self, iterations, intTime, bitDepth, overlap, timeout, pileup, im_width):
        ...
        # time.sleep(1) # added to ensure proper command sequence
    ...
    def get_gated_intensity(self, bitDepth, intTime, iterations, gate_steps, gate_step_size, gate_step_arbitrary, gate_width, 
        ...
        # --- 1. PREVENT SOCKET CLOGGING ---
        # Clear any leftover data (like the PNG file paths) from the buffer
        self.t.setblocking(0)
        try:
            while self.t.recv(4096): pass
        except:
            pass
        ...
        # We force 'stream' to 1 to ensure the camera sends raw binary data, not file paths.
        # We use .0f for gate_offset to ensure no scientific notation is sent to hardware.
        command_str = (f"G,{int(bitDepth)},{int(intTime)},{int(iterations)},{int(gate_steps)},"
                    f"{int(gate_step_size)},{int(gate_step_arbitrary)},{int(gate_width)},"
                    f"{gate_offset:.0f},{int(gate_direction)},{int(gate_trig)},"
                    f"{int(overlap)},1") # The '1' at the end is the 'stream' fix
        
        # --- 4. HARDWARE HANDSHAKE ---
        self.t.send(bytes(f"PU,{int(pileup)}", "utf-8"))
        self.t.recv(1024) # Capture the ACK for the PU command
        
        self.t.send(bytes(command_str, "utf-8"))
        
        # --- 5. DATA COLLECTION ---
        data = bytearray()
        expected_frames = iterations * gate_steps
        
        # Determine expected byte size to prevent reshape errors
        bytes_per_pixel = 1 if (bitDepth < 9 and pileup == 0) else 2
        expected_bytes = self.row * im_width * expected_frames * bytes_per_pixel

        while True:
            datablock = self.t.recv(65536) # Increased buffer size for speed
            data.extend(datablock)
            if data[-4:] == b"DONE":
                break
            # Emergency exit: if we stop receiving data for a while
            if len(datablock) == 0: break

        # Remove the "DONE" tag
        raw_payload = data[:-4]

        # --- 6. SAFETY CHECK (The Reshape Fix) ---
        if len(raw_payload) < expected_bytes:
            print(f"CRITICAL: Hardware returned {len(raw_payload)} bytes, expected {expected_bytes}.")
            # Detect if the hardware sent an error string instead of pixels
            try:
                print(f"Hardware message: {raw_payload.decode('utf-8')}")
            except:
                pass
            # Pad with zeros to prevent the "reshape" crash
            raw_payload = raw_payload.ljust(expected_bytes, b'\x00')

        # --- 7. IMAGE RECONSTRUCTION ---
        # Convert to numpy and reshape
        if bytes_per_pixel == 1:
            # 8-bit mode
            np_data = np.frombuffer(raw_payload[:expected_bytes], dtype=np.uint8)
            img = np_data.reshape((self.row, im_width, expected_frames), order='F')
        else:
            # 16-bit or Pile-up mode
            np_data = np.frombuffer(raw_payload[:expected_bytes], dtype=np.uint8)
            # Even/Odd byte interleaving for 16-bit reconstruction
            even = np_data[0::2].astype(np.uint16)
            odd = np_data[1::2].astype(np.uint16)
            combined = (odd << 8) | even
            img = combined.reshape((self.row, im_width, expected_frames), order='F')
    ...
    #     # 3. Send Gated Command (Force integers where required to prevent firmware crashes)
    #     command = bytes("G,{},{},{},{},{},{},{},{},{},{},{},{}".format(
    #         int(bitDepth), float(intTime), int(iterations), int(gate_steps), 
    #         float(gate_step_size), int(gate_step_arbitrary), int(gate_width), 
    #         int(gate_offset), int(gate_direction), int(gate_trig), 
    #         int(overlap), int(stream)), "utf8")
        
    #     self.t.send(command)

    #     # 4. Robust Data Receiving (Fixes the "DONE" Desync)
    #     data = bytearray()
    #     expected_end = b"DONE"
        
    #     while True:
    #         try:
    #             datablock = self.t.recv(1048576) # Increased buffer to 1MB for speed
    #             if not datablock:
    #                 print("Connection closed by server.")
    #                 break
    #             data.extend(datablock)
                
    #             # Check the end of the buffer for "DONE" 
    #             # We search the last 10 bytes in case "DONE" is followed by a newline
    #             if expected_end in data[-10:]:
    #                 break
    #         except Exception as e:
    #             print(f"Socket error during receive: {e}")
    #             break

    #     # Clean the "DONE" marker and any trailing whitespace
    #     done_idx = data.find(b"DONE")
    #     if done_idx != -1:
    #         clean_data = data[:done_idx]
    #     else:
    #         clean_data = data

    #     # 5. Pre-allocate Output Array
    #     total_frames = iterations * gate_steps
    #     if bitDepth == 4 or (bitDepth < 9 and pileup == 0):
    #         img = np.zeros([self.row, im_width, total_frames], dtype=np.uint8)
    #         bytes_per_frame = self.row * im_width
    #     else:
    #         img = np.zeros([self.row, im_width, total_frames], dtype=np.uint16)
    #         bytes_per_frame = self.row * im_width * 2

    #     # 6. Optimized Data Parsing
    #     # Converting to numpy once is MUCH faster and safer than slicing bytearrays in a loop
    #     raw_np = np.frombuffer(clean_data, dtype=np.uint8)

    #     for i in range(total_frames):
    #         start = i * bytes_per_frame
    #         end = start + bytes_per_frame
            
    #         # Prevent IndexError if data is shorter than expected
    #         if end > len(raw_np):
    #             print(f"Warning: Received truncated data at frame {i}")
    #             break
                
    #         frame_data = raw_np[start:end]

    #         if bytes_per_frame == self.row * im_width * 2:
    #             # 16-bit reconstruction (Pileup or high bit-depth)
    #             # Evens are low byte, Odds are high byte
    #             even = frame_data[::2].astype(np.uint16)
    #             odd = frame_data[1::2].astype(np.uint16)
    #             img[:, :, i] = ((odd << 8) | even).reshape((self.row, im_width))
    #         else:
    #             # 8-bit reconstruction
    #             img[:, :, i] = frame_data.reshape((self.row, im_width))
