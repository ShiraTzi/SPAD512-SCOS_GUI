...
class SensitivityTestAnalyzer:
    def __init__(self, base_path, output_csv="sensitivity_test_results.csv", recalc_SCOS=False, window_seconds=30.0, metric="contrast", window_cut_transition_seconds=5.0):
    ...
    def calibrate_intensity_artifact(self):
        """Pass 1: Learn the absolute intensity jump Delta_I(t) from Gated measurements."""
        ...
        for folder_path in self.find_measurement_folders():
            if self.extract_method_label(folder_path) == "Gated":
                metadata = self.load_metadata(folder_path)
                gate_position_ps = metadata.get("gate_position_ps")
                if gate_position_ps is None: continue
                gate_position_ps = float(gate_position_ps)
                time_gate_ps = round(gate_position_ps - time_gate_reference_ps, 4)
                ...
                if time_vec is None or intensity is None: continue
                    
                time_vec = np.asarray(time_vec).flatten()
                intensity = np.asarray(intensity).flatten()

                # Calculate baseline mean
                base_mask = (time_vec >= time_vec[0]) & (time_vec < time_vec[0] + self.window_seconds)
                base_I = np.nanmean(intensity[base_mask])

                tourniquet_mask = (time_vec >= time_vec[0] + self.window_seconds + self.window_cut_transition_seconds) & (time_vec < time_vec[0] + 2*self.window_seconds)
                tourniquet_I = np.nanmean(intensity[tourniquet_mask])
                # Calculate absolute dynamic jump
                delta_I = tourniquet_I - base_I

                gated_delays.append(time_gate_ps)
                gated_base_Is.append(base_I)
                gate_tourniquet_Is.append(tourniquet_I)
                delta_profiles.append(delta_I)
                time_arrays.append(time_vec)
                
        #plot delta_profiles for all gated measurements to visualize the artifact behavior across gate positions
        # plt.figure(figsize=(10, 6))
        # plt.plot(gated_delays, delta_profiles, "o", label="Delta I vs Gate Position")
        ...
        # plt.show()
        # if not gated_delays:
        #     print("No Gated measurements found to calibrate intensity artifact.")
        #     return
        # if there are the same gated_delays, we need to average their base_Is and delta_profiles before interpolation
        
        from scipy.interpolate import interp1d

        # Model 1: Interpolate pure sample Baseline Intensity across Gate Positions
        sorted_indices = np.argsort(gated_delays)
        delays_sorted = np.array(gated_delays)[sorted_indices]
        base_Is_sorted = np.array(gated_base_Is)[sorted_indices]
        tourniquet_Is_sorted = np.array(gate_tourniquet_Is)[sorted_indices]
        deltas_sorted = np.array(delta_profiles)[sorted_indices]
        ...
        unique_deltas = deltas_sorted[unique_indices]
        
        if len(unique_delays) > 1:
            self.gated_baseline_model = interp1d(unique_delays, (unique_tourniquet_Is)/unique_base_Is, kind='linear', fill_value="extrapolate")
        else:
            self.gated_baseline_model = lambda x: (unique_tourniquet_Is[0])/unique_base_Is[0]

        # plot the interpolated baseline model
        plt.figure(figsize=(10, 6))
        # plt.plot(unique_delays, unique_base_Is, "o", label="Measured Baseline I")
        # plt.plot(unique_delays, unique_deltas, "o", label="Measured Delta I")
        delay_range = np.linspace(min(unique_delays), max(unique_delays), 100)
        interpolated_base_Is = self.gated_baseline_model(delay_range)
        plt.plot(delay_range, interpolated_base_Is, "-", label="Interpolated Baseline I")
        plt.xlabel("Gate Position (ps)")
        plt.ylabel("Intensity / Baseline Intensity")
        plt.title("Intensity Artifact Calibration")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
        # # Model 2: Generate the universal Delta I(t) temporal template
        # self.common_time_vec = time_arrays[0]
        # max_len = min(len(p) for p in delta_profiles)
        # stacked_profiles = np.vstack([p[:max_len] for p in delta_profiles])
        # self.universal_delta_I_profile = np.nanmean(stacked_profiles, axis=0)
        # self.common_time_vec = self.common_time_vec[:max_len]
        
        print(" Intensity artifact calibration complete.")
    ...
    def plot_results(self, time_vec, rbfi, save_path, filename, title_suffix="", base_mask=None,tourniquet_mask=None, late_mask=None):
        try:
            ...
            if tourniquet_mask is not None and np.any(tourniquet_mask):
                ax.axvspan(time_vec[tourniquet_mask][0], time_vec[tourniquet_mask][-1], color="lightblue", alpha=0.12, label="Tourniquet window")
    ...
    def process_measurement(self, folder_path, force_recalc=False):
        ...
        try:
            ...
            intensity = np.asarray(self.load_npy(folder_path, "intensity.npy")).flatten()
            ...
            tourniquet_end = min(tourniquet_start + self.window_seconds- self.window_cut_transition_seconds, end_time)
            late_end = end_time
            ...
            # Let R be the ratio of Sample Intensity during Tourniquet to Baseline
            R = 1.0 
            
            if method_label == "Gated":
                # Gated SCOS uses actual measured deviation
                base_I = np.nanmean(intensity[base_mask])
                tourniquet_I = np.nanmean(intensity[(time_vec >= tourniquet_start) & (time_vec < tourniquet_end)])
                if base_I != 0 and np.isfinite(base_I):
            ...
            elif method_label in ["ISCOS", "PaLS-iSCOS"]:
                # ISCOS requires interpolated sample intensity
                gate_to_query = coherence_gate_ps if coherence_gate_ps is not None else 0
                try:
                    R = float(self.gated_baseline_model(gate_to_query))
                except:
                    R = 1.0  # Fallback to prevent divide-by-zero
                
                # Heterodyne physics:
                contrast_multiplier = np.sqrt(R) if R > 0 else 1.0
                rbfi_multiplier = 1/R if R > 0 else 1.0
                
            else:
                contrast_multiplier = 1.0
                rbfi_multiplier = 1.0
            
            print("FIX FOR INTENSITY IS OFF")
            contrast_multiplier = 1.0
            rbfi_multiplier = 1.0
            
            
            print(f"  Contrast multiplier: {contrast_multiplier:.6g}, rBFI multiplier: {rbfi_multiplier:.6g}")
            ...
            late_intensity_summary = self.summarize_window(intensity, time_vec, late_start, late_end)
            ...
            if self.metric == "rbfi":
                ...
                percentage_rbfi_difference = float((normalized_tourniquet_mean - 1.0) * 100.0) if np.isfinite(normalized_tourniquet_mean) else np.nan
            ...
            saved_tourniquet_mean = normalized_tourniquet_mean
            saved_tourniquet_std = normalized_tourniquet_std

            saved_baseline_mean = base_summary["mean"]
            saved_baseline_std = base_summary["std"]
            self.results.append({
                "folder": folder_label,
                ...
                "raw_late_mean_contrast": late_contrast_summary["mean"],
                "raw_late_std_contrast": late_contrast_summary["std"],
                ...
                "raw_late_mean_intensity": late_intensity_summary["mean"],
                "raw_late_std_intensity": late_intensity_summary["std"],
                ...
                "raw_late_mean_rBFI": late_rbfi_summary["mean"],
                "raw_late_std_rBFI": late_rbfi_summary["std"],
                "correction_factor_rBFI": rbfi_multiplier,
                "correction_factor_contrast": contrast_multiplier,
                "metric": self.metric,
                "total_frames": int(time_vec.size),
                
                "baseline_mean_normalized": saved_baseline_mean,
                "baseline_std_normalized": saved_baseline_std,

                "tourniquet_mean_normalized": saved_tourniquet_mean,
                "tourniquet_std_normalized": saved_tourniquet_std,
            })
            ...
            print(f"  Normalized tourniquet contrast mean/std: {tourniquet_summary['mean']:.6g} / {tourniquet_summary['std']:.6g}")
    ...
    def run(self):
        ...
        if not measurement_folders:
            ...
            return
        # calibrate intensity artifact from Gated measurements before processing any folders, so we can apply corrections to all measurements in one pass
    ...
    def aggregate_and_plot(self, df):
        ...
        for _, row in group.iterrows():
            ...
            tourniquet_start = row.get('tourniquet_start_s')
            tourniquet_end = row.get('tourniquet_end_s')

            key = (method, offset)

            sample_buckets.setdefault(key, []).append({
                "pct_mean": row["percentage_rBFI_difference"],

                "baseline_mean": row["baseline_mean_normalized"],
                "baseline_std": row["baseline_std_normalized"],

                "tourniquet_mean": row["tourniquet_mean_normalized"],
                "tourniquet_std": row["tourniquet_std_normalized"],
            })
